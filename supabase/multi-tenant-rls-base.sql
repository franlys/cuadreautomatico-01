-- ============================================
-- Multi-Tenant Platform - Row Level Security Base
-- ============================================
...
COMMENT ON FUNCTION auth.get_user_empresa_id() IS 
...
CREATE POLICY "tenant_isolation_select_perfiles"
  ...
  USING (empresa_id = auth.get_user_empresa_id());
...
CREATE POLICY "tenant_isolation_update_perfiles"
  ...
  USING (
    id = auth.uid() 
    AND empresa_id = auth.get_user_empresa_id()
  );
...
CREATE POLICY "tenant_isolation_insert_empleados"
  ...
  WITH CHECK (empresa_id = auth.get_user_empresa_id());
...
-- NOTAS IMPORTANTES
-- ============================================
-- 1. Todas las políticas filtran por empresa_id usando auth.get_user_empresa_id()
